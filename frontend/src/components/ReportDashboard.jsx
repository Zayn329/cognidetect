...
const ReportDashboard = ({ 
  ...
  phonologyScore,
  readingScore,
  writingScore, // Expecting "Risk Score" (High = Bad)
  // Navigation
  onStartDyslexia 
}) => {
  ...
  const pScore = phonologyScore || 0; // Raw: 0 to 5 (Performance)
  const rScore = readingScore || 0;   // Raw: Risk %
  const wScore = (writingScore !== undefined && writingScore !== null) ? writingScore : 0; // Raw: Risk %
  ...
  // DYSLEXIA CONSENSUS
  // We must normalize Phonology because it is a "Performance" score (High = Good).
  // Formula: (Max - Score) / Max * 100
  // Example: 3/5 Correct -> 2/5 Wrong -> 40% Risk
  const pRisk = pScore > 0 ? ((5 - pScore) / 5) * 100 : 0;
  
  // Reading & Writing are already "Risk" scores (High = Bad)
  const rRisk = rScore;
  const wRisk = wScore;
  ...
  const handleSave = async () => {
    ...
    try {
      await API.post('/save-result', {
        ...
        dyslexia_risk: dyslexiaOverall, // The calculated average
        notes: getNote()
      });
    ...
    <div className="space-y-8 fade-in-up">
      ...
      <div className="flex justify-center mb-6">
        <div className="bg-slate-100 p-1 rounded-xl inline-flex shadow-inner">
          <button onClick={() => setActiveTab('dementia')} className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'dementia' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>🧠 Dementia Screening</button>
          <button onClick={() => setActiveTab('dyslexia')} className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'dyslexia' ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>📖 Dyslexia Screening</button>
        </div>
      </div>
      ...
      <div className="bg-card p-8 rounded-3xl border border-border shadow-xl relative overflow-hidden">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
               ...
               <div className="p-6 bg-orange-500/5 rounded-2xl border border-orange-500/10 text-center">
                 ...
                 <div className="text-4xl font-black text-foreground">
                    {/* Display score if valid, else dash */}
        ...
        {/* DEMENTIA GRID (Hidden when inactive) */}
        {activeTab === 'dementia' && (
